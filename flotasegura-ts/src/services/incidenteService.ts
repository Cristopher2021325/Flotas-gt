...
export async function obtenerIncidentes(): Promise<incidente[]> {
  const [rows] = await pool.query<RowDataPacket[]>("SELECT * FROM incidentes");
...
export async function obtenerIncidentePorId(id: string): Promise<incidente> {
  ...
  if (rows.length === 0) throw new Error(`no se encontro un incidente con el id "${id}"`);

  return rows[0] as incidente;
}
...
export async function reportarIncidente(datos: Omit<incidente, "id" | "ocurridoEn">): Promise<incidente> {
  ...
  const fechaOcurrencia = new Date().toISOString();

  const [resultado] = await pool.query<ResultSetHeader>(
    "INSERT INTO incidentes (viajeId, conductorId, vehiculoId, descripcion, severidad, ocurridoEn) VALUES (?, ?, ?, ?, ?, ?)",
    [
      datos.viajeId,
      datos.conductorId,
      datos.vehiculoId,
      datos.descripcion,
      datos.severidad || "leve",
      fechaOcurrencia,
    ]
  );
